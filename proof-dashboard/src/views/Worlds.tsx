...
function WorldCard({ w, origin }: { w: World; origin: DataOrigin }) {
  ...
  return (
    <Card
      ...
      actions={
        <>
          <OriginBadge origin={origin} dataset="World" compact />
...
export function Worlds() {
  ...
  const origins =
    data.status === "ready"
      ? [{ dataset: "Worlds", origin: data.data.origin }]
      : data.status === "error"
        ? [{ dataset: "Worlds", origin: data.origin }]
        : [];
      ...
      {data.status === "loading" ? (
        ...
        <div className="grid cols-12">
          {data.data.worlds.map((w) => (
            <WorldCard key={w.id} w={w} origin={data.data.origin} />
          ))}
        </div>
      )}
    </div>
