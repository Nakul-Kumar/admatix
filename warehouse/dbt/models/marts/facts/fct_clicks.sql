{{ config(materialized='incremental', unique_key='click_id') }}
...
with base as (
  select
    ...
    (s.metric_date::timestamptz + interval '12 hours') as click_ts,
    s.clicks,
    case when s.clicks > 0 then (s.spend / s.clicks)::numeric(18,6) else 0::numeric(18,6) end as cost,
    s._source
...
select
  row_number() over (order by date_key, campaign_key) as click_id,
  date_key,
  account_key,
  campaign_key,
  ad_set_key,
...
{% if is_incremental() %}
  and not exists (
    ...
    where existing.date_key = base.date_key
      ...
      and existing.click_ts = base.click_ts
  )
{% endif %}
