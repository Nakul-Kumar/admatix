...
def _dispatch_method(method: str, req, events):
...
def run_rmse_bias(config: ValidationConfig) -> RmseBiasResult:
    ...
    for cell in enumerate_cells(config.world_grid, config.seeds):
        ...
        if config.verifier_method == "auto":
            method = select_method(req, events)
        else:
            method = config.verifier_method
        ...
        runs.append(
            WorldRun(
                ...
                diagnostics={"n_users": int(n_users)},
            )
        )
