...
def _skipped_llm_run(cfg: ArmRunConfig) -> RunResult:
    return RunResult(
        config=cfg,
        env_config_summary={},
        decision_timeline=[],
        ...
        notes=["llm_run_skipped_by_operator_flag"],
        row_kind="llm_skipped",
        row_status="skipped",
    )
...
def run_matrix(
    *,
    ...
    for world in world_types:
        for seed, buyer_kind in runs_per_tier:
            for arm in arms:
                ...
                fingerprints[key] = fp
                buyer = buyer_factory(arm_meta["skill_tier"], buyer_kind, model)
                ...
                if buyer_kind == "llm" and isinstance(buyer, ClaudeHeadlessBuyer):
                    if buyer.used_fallback:
                        result.row_kind = "llm_fallback"
                        result.row_status = "fallback"
                        result.notes.append(
    ...
    # ---- scorecard ----
    completed_runs = [r for r in runs if r.row_status == "completed" or r.row_status == "fallback"]
    summary_seed = (seeds_policy or seeds_llm or [17])[0]
    summary_run = completed_runs[0] if completed_runs else None
    config_summary = {
        "seeds_llm": [] if skip_llm else seeds_llm,
        "seeds_policy": seeds_policy,
        "n_periods": summary_run.env_config_summary["n_periods"] if summary_run else 0,
        "decision_every_n_days": summary_run.env_config_summary["decision_every_n_days"]
        if summary_run
        else 0,
        "world_types": world_types,
        "campaigns_per_account_by_world": {
            w: len(build_env_config(w, summary_seed, data_dir=data_dir).campaigns)
