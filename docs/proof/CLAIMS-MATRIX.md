...
| Supabase data layer | migrations/dbt/ledger schemas on `main`; Supabase connected on VPS; `0005_live_data_readiness.sql` validated in disposable Postgres | IMPLEMENTED IN REPO; LIVE 0005 NOT APPLIED | AdMatix has a managed Postgres/Supabase data-layer shape for ledger, app, warehouse, simulator, benchmark, plus a validated repo migration for shadow connector syncs, experiment designs, and proof bundles. | "Production multi-tenant SaaS isolation is fully proven" or "`0005` has been applied to live Supabase." |
